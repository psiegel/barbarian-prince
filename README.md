...
Optionally, drop a `data/map-data.csv` of hex terrain and features in place and run
`python3 tools/extract_map.py` — the CLI then takes hex ids directly. The extractor
audits that CSV against the booklet rather than trusting it, and reports whatever
disagrees.
...
data/map-data.csv    third-party terrain/feature transcription (not tracked)
