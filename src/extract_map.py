...
def check_edges(hexes: dict[str, dict]) -> tuple[dict, dict, list[str], list[str]]:
    ...
    for hid in sorted(hexes):
        nb = procedures.neighbours(*procedures.parse_hex(hid))
...
def _linked(hexes: dict[str, dict], a: str, b: str, kind: str) -> bool:
    if a not in hexes or b not in hexes:
        return False
    for d, h in procedures.neighbours(*procedures.parse_hex(a)).items():
        if h == b:
...
def main(argv: list[str]) -> int:
    ...
    payload = {
        "_comment": ("Generated by src/extract_map.py from data/map-data.csv, a "
